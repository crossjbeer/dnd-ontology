...
- Owlready2 + HermiT (`sync_reasoner_hermit`)

Other reasoners worth considering:
- Pellet (Java OWL DL reasoner)
...
def _load_ttl(ttl_path: Path) -> int:
	"""Read a Turtle graph from disk and return triple count."""
	graph = Graph()
...
def reason_over_ontology(
	...
	if asserted_ttl is not None and asserted_ttl.exists():
		...
		triples_before = len(build_rdflib_graph(world))
